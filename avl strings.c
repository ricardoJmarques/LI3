...
BTree insertAVL(BTree t, char *str, int *grow) { //função principal de inserção na AVL
	...
	*grow = 1;
		...
		if (strcmp(t->string , str) < 0) // testa se o valor a inserir e maior que o valor do nodo
				t = insertRight(t,str,grow); // insere a direita
		else
			t = insertLeft(t,str,grow); // insere a esquerda se o valor a inserir for menor que o valor do nodo
	return t;
}

BTree insertRight(BTree t, char* str, int *grow) { //insere no ramo da direita
	t->right = insertAVL(t->right,str,grow); // procura na arvore uma folha adequada
...
BTree insertLeft(BTree t, char* str, int *grow) { //insere no ramo da esquerda
	t->left = insertAVL(t->left,str,grow); //procura na arvore um folha adequada
	if (*grow)	// a variável grow assinala o crescimento da arvore, ou seja, quando for adicionado um nodo (fazer malloc) grow passa a 1
		switch (t->balance) {
